...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  8 17:45:40 2025
...
parser.add_argument('--out_folder', default= 'pred_out_100125_ftnep', type=str, help='out folder name')
...
if __name__ == '__main__':
    ...
    for f in fold_list:
        ...
        #TODO: Use test to fine-tune
        train_data, train_sp_ids, train_pt_ids, train_cohorts, train_coords  = nep_split['val']
        test_data, test_sp_ids, test_pt_ids, test_cohorts, _ = nep_split['train']
        val_data, val_sp_ids, val_pt_ids, val_cohorts,_ = nep_split['test']
        
        
        # NEP all
        test_data2, test_sp_ids2, test_pt_ids2, test_cohorts2 = combine_all(nep_split)
        test_data_final   = test_data + val_data
        test_sp_ids_final =  test_sp_ids +  val_sp_ids
        test_pt_ids_final = test_pt_ids + val_pt_ids
        test_cohorts_final  = test_cohorts + val_cohorts
        ...
        checkpoint = torch.load(os.path.join(outdir1.replace("_ftnep", ""),'checkpoint'+ str(mode_idxes) + '.pth'))
        ...
        label_freq_array = np.array([0.97, 0.03])
        ...
        avg_loss, pred_df, pref_tb  = run_eval(train_data, train_sp_ids, "TRAIN", 
                                               loss_fn, model= model, 
                                               device = device,logit_adj_infer = True, 
                                               logit_adj_train = args.logit_adj_train, 
                                               logit_adjustments=logit_adjustments, l2_coef = args.l2_coef )
        best_th = pref_tb['best_thresh'].item()
        
        #Best thresd hodl fro PR_curve
        from sklearn.metrics import precision_recall_curve
        y_true = pred_df['True_y']
        y_pred_proba = pred_df['adj_prob_1']

        precision, recall, thresholds = precision_recall_curve(y_true, y_pred_proba)
        f1 = 2 * (precision * recall) / (precision + recall)
        best_th = thresholds[f1.argmax()]
        
        avg_loss, pred_df_test, pref_tb_test = run_eval(test_data_final, test_sp_ids_final, "Nep_test",     
        ...
        comb_perf = pd.concat([pref_tb, pref_tb_test])
        ...
        avg_loss, pred_df_val, pref_tb_val = run_eval(val_data, val_sp_ids, "Nep_valid",     
                                              loss_fn, model= model, device = device,logit_adj_infer = True, 
                                              logit_adj_train = args.logit_adj_train, 
                                              logit_adjustments=logit_adjustments, 
                                              l2_coef = args.l2_coef, pred_thres = best_th)
        avg_loss, pred_df_test, pref_tb_test = run_eval(test_data, test_sp_ids, "Nep_test",     
                                              loss_fn, model= model, device = device,logit_adj_infer = True, 
                                              logit_adj_train = args.logit_adj_train, 
                                              logit_adjustments=logit_adjustments, 
                                              l2_coef = args.l2_coef, pred_thres = best_th)

        comb_perf = pd.concat([pref_tb, pref_tb_val, pref_tb_test])
        comb_perf.to_csv(os.path.join(outdir5, "after_finetune_performance.csv"))
        
        
        pred_df_test.to_csv(os.path.join(outdir4, "after_finetune_prediction.csv"))

        
        #get attention:
        for (i, parts) in enumerate(zip(test_data, test_sp_ids)):
            x, y, tf, sl = parts[0]
            ...
            tile_info = [d['tile_info'] for d in nep_union_ol0 if d['sample_id'] == sp_id][0]
