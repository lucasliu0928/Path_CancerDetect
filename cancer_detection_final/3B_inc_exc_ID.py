...
from Preprocessing import prepross_neptune_label_data, extract_before_second_underscore, preproposs_tcga_label, build_info_path, filter_cancer_ids
...
label_path = os.path.join(proj_dir,'intermediate_data','3_labels', args.cohort_name)
info_path_st1_ol100 = build_info_path(proj_dir, args.cohort_name, 100, stnorm=True)
info_path_st1_ol0   = build_info_path(proj_dir, args.cohort_name, 0,   stnorm=True)
info_path_st0_ol100 = build_info_path(proj_dir, args.cohort_name, 100, stnorm=False)
info_path_st0_ol0   = build_info_path(proj_dir, args.cohort_name, 0,   stnorm=False)
...
if args.cohort_name.replace("z_nostnorm_", "") == "OPX":
    ...
    label_df = pd.read_csv(os.path.join(label_path, "label_df_allslides.csv")) #274 Samples, 272 patient, #New data (there are some ids in old data exclude due to bad quality)
    ids_high_quality = list(label_df[id_col].unique()) 
    ...
    label_df = pd.read_csv(os.path.join(label_path, "label_df_allslides.csv")) #274 Samples, 272 patient, #New data (there are some ids in old data exclude due to bad quality)
    ids_high_quality = list(label_df[id_col].unique()) 
    selected_ids = [x for x in selected_ids if x in ids_high_quality] #447
...
results = filter_cancer_ids(
    ...
    threshold=  args.TUMOR_FRAC_THRES,
    id_col=id_col,
    cohort_name = args.cohort_name
)

common_ids = results["common_ids"]

#Exclude the non-cancer samples from all methods
selected_ids = list(common_ids)
selected_ids.sort()


#Updated label
label_df = label_df.loc[label_df[id_col].isin(selected_ids)]
print(f"Final {args.cohort_name} SAMPLE IDs (n = ", len(set(label_df['SAMPLE_ID'])), ")")
print(f"Final {args.cohort_name} PTS IDs (n = ", len(set(label_df['PATIENT_ID'])), ")")
label_df.to_csv(os.path.join(out_location, "final_sample_label_df.csv"), index = False)
