...
from data_loader import merge_data_lists_h5, load_dataset_splits, merge_data_lists_h5_parallel, save_merged_to_h5, merge_data_lists_h5_parallel_v2
...
# #FOR MIL-Lab
sys.path.insert(0, os.path.normpath(os.path.join(os.getcwd(), '..', '..', 'other_model_code','MIL-Lab',"src","models")))
from abmil import ABMILModel
...
if __name__ == '__main__':
    ...
    data_out_dir = os.path.join(proj_dir, "intermediate_data", "7_final_model_ready_data", args.fe_method)
    ...
    #Ouput merged data
    ##########################################################################################
    from data_loader import merge_data_lists_h5_to_file
    start = time.time()
    merge_data_lists_h5_to_file(opx_ol0_nst, opx_ol0, data_out_dir + "/opx_ol0_merged.h5", merge_type="union", max_workers=4)
    end = time.time()
    print(f"Merge finished in {end - start:.2f} seconds")
    
    
    start = time.time()
    merge_data_lists_h5_to_file(opx_ol100_nst, opx_ol100, data_out_dir + "/opx_ol100_merged.h5", merge_type="union", max_workers=4)
    end = time.time()
    print(f"Merge finished in {end - start:.2f} seconds")
    
    
    start = time.time()
    merge_data_lists_h5_to_file(tcga_ol100_nst, tcga_ol100, data_out_dir + "/tcga_ol100_merged.h5", merge_type="union", max_workers=4)
    end = time.time()
    print(f"Merge finished in {end - start:.2f} seconds")
    
    
    start = time.time()
    merge_data_lists_h5_to_file(tcga_ol0_nst, tcga_ol0, data_out_dir + "/tcga_ol0_merged.h5", merge_type="union", max_workers=4)
    end = time.time()
    print(f"Merge finished in {end - start:.2f} seconds")
    
    
    # start = time.time()
    # check = H5Cases(data_out_dir + "/opx_ol0_merged.h5")
    # end = time.time()
    # print(f"Merge finished in {end - start:.2f} seconds")
