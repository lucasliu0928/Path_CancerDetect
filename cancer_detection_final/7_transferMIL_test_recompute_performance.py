...
if __name__ == '__main__':
    ...
    for f in fold_list:
        ...
        for tf in tf_list:
            ...
            outdir_list = [outdir0,outdir1,outdir4,outdir5, outdir6]
            ...
            ###filtered 0.9
            test_data1, mask1 = filter_by_tumor_fraction(test_data1, threshold = args.tumor_frac)
            test_data2, mask2 = filter_by_tumor_fraction(test_data2, threshold = args.tumor_frac)
            test_data3, mask3 = filter_by_tumor_fraction(test_data3, threshold = args.tumor_frac)
            test_data4, mask4 = filter_by_tumor_fraction(test_data4, threshold = args.tumor_frac)
            test_data5, mask5 = filter_by_tumor_fraction(test_data5, threshold = args.tumor_frac)
            test_data6, mask6 = filter_by_tumor_fraction(test_data6, threshold = args.tumor_frac)
            ...
            pred_list = [pred_df_val, pred_df4, pred_df5, pred_df2, pred_dff]
            cohort_list = ["OPX_TCGA_VAL",'OPX_test', 'TCGA_test','NEP_ALL',"OPX_TCGA_TEST_and_NEP_ALL"]
            ...
            for cur_pred_df, name in zip(pred_list,cohort_list):
                ...
                #compute the numer of negative
                True_y = cur_pred_df['True_y'] == 0
                n_neg = True_y.sum()
                prop_neg = n_neg / n_total*100
                cur_ci_df['N NEG (%)'] = f'{n_neg} ({prop_neg: .2f}%)'
                
                cur_ci_df['TOTAL'] = f'{n_total}'
            ...
            # #get attention:
            # generate_attention_csv(model, test_data4, test_sp_ids4, mask4, opx_union_ol0, outdir6)
            # generate_attention_csv(model, test_data5, test_sp_ids5, mask5, tcga_union_ol0, outdir6)
            # generate_attention_csv(model, test_data2, test_sp_ids2, mask2, nep_ol0, outdir6)
