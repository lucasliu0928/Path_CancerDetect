...
#Run: python3 -u 7_train_ACMIL_mixed.py --train_cohort TCGA_OPX 
############################################################################################################
#Parser
############################################################################################################
        ...
        for focal_alpha, focal_gamma in a_g_repairs:
            ...
            if has_seven_csv_files(outdir55) == False:
                ...
                for epoch in range(train_epoch):
                    ...
                    test_auc1, test_acc1, test_f11, test_loss1 = evaluate_multitask(model, criterion, test_loader1, device, conf, 'Test', use_sep_criterion = args.use_sep_cri)
                ...
                y_pred_tasks_test, y_predprob_task_test, y_true_task_test = predict_v2(model2, test_loader1, device, conf, 'Test')
                ...
                #           Test 2
                ###################################################       
                y_pred_tasks_test, y_predprob_task_test, y_true_task_test = predict_v2(model2, test_loader2, device, conf, 'Test')
                y_pred_tasks_val,  y_predprob_task_val, y_true_task_val = predict_v2(model2, val_loader, device, conf, 'Test')
