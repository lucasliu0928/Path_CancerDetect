...
save_location = cur_wd + 'intermediate_data/cancer_prediction_results/'  # args.save_location
save_location2 = cur_wd + 'intermediate_data/tiles/'  # args.save_location
save_location3 = cur_wd + 'intermediate_data/tiles_info/'  # args.save_location
...
model_path2 = cur_wd + 'code_s/cancer_detection/dlv3_2ep_2e4_update-07182023_RT.pkl'
...
selected_ids = ['OPX_005', 'OPX_006' , 'OPX_007' , 'OPX_008', 'OPX_009', 'OPX_010', 'OPX_011' , 'OPX_012', 'OPX_013', 'OPX_014', 'OPX_015']

for cur_id in selected_ids:
    #cur_id = 'OPX_004'
    _file = "/fh/scratch/delete90/etzioni_r/lucas_l/michael_project/mutation_pred//data/OPX/" + cur_id + ".tif"
                    ...
                    if tile_tiss > 0.9:
                        ...
                        if ws < 0.9:
                            ...
                            try: 
                                ...
                                x_map[int(np.floor(tile_coords[0][1] / lvl_resize)):int(np.floor(tile_ends[1] / lvl_resize)),int(np.floor(tile_coords[0][0] / lvl_resize)):int(np.floor(tile_ends[0] / lvl_resize))] += outputs_np[1]
                            except:
                                pass

                            #Save tiles if pred > 0.8
                            cur_xmap = x_map[int(np.floor(tile_coords[0][1] / lvl_resize)):int(np.floor(tile_ends[1] / lvl_resize)),int(np.floor(tile_coords[0][0] / lvl_resize)):int(np.floor(tile_ends[0] / lvl_resize))]
                            cur_x_count = x_count[int(np.floor(tile_coords[0][1] / lvl_resize)):int(np.floor(tile_ends[1] / lvl_resize)),int(np.floor(tile_coords[0][0] / lvl_resize)):int(np.floor(tile_ends[0] / lvl_resize))]
                            cur_x_count = np.where(cur_x_count < 1, 1, cur_x_count)
                            cur_xmap = cur_xmap / cur_x_count
                            single_tile = tiles.get_tile(tile_lvls.index(lvl), (x, y))
                            single_tile_RGB = single_tile.convert('RGB')
                            img_test = np.array(single_tile_RGB.resize((save_image_size, save_image_size), resample=PIL.Image.LANCZOS))

                            if np.any(cur_xmap > 0.8):
                                #print(x,y)
                                tile_count += 1
                                selected = 1
                                plt.imsave(cur_save_loc + cur_outname + "_GT08.png",img_test,dpi = 100)
                            else:
                                selected = 0
    ...
    slide_ROIS(polygons=tissue, mpp=float(oslide.properties[openslide.PROPERTY_NAME_MPP_X]),
                    savename=os.path.join(save_location,save_name+'_tissue.json'), labels='tissue', ref=[0,0], roi_color=-16770432)


    #TODO: if no predicted high value, then random generate patches 
