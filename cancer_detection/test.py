...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  9 15:03:15 2024
...
#     def __init__(self):
#         save_location = cur_wd + '/Data/cancer_output/'  # args.save_location
...
#         pixel_overlap = 50  # specify the level of pixel overlap in your saved images
#         limit_bounds = True  # this is weird, dont change it
#         tiff_lvl = 4 # low res pyramid level to grab
#         model_path2 = cur_wd + '/Code_Stephanie/cancer_detection/dlv3_2ep_2e4_update-07182023_RT.pkl'
...
#         learn = load_learner(model_path2,cpu=False) #Change from cpu = False, cuz CUDA version for pytorch needs to be updated

#         for _file in flist:
#             print(_file)
#             _file = nprossessed_flist[0]
...
#                 # pull tiles from levels specified by mag_extract
...
#                                         #tile_copy = tile_copy.resize(size=(save_image_size, save_image_size),resample=PIL.Image.ANTIALIAS)
#                                         #tile_pull = tile_pull.resize(size=(run_image_size, run_image_size),resample=PIL.Image.ANTIALIAS)
...
#                                         with learn.no_bar():
#                                             pred_class, pred_idx, outputs = learn.predict(tile_pull[0:2, 0:2, 0:3])
#                                         outputs_np = outputs.numpy()
...
#                 # slideimg.save(os.path.join(save_location, save_name + '_cancer_prob.jpeg'))
#                 # lvl_img.save(os.path.join(save_location, save_name + '_lowres.tiff'))
...
#                 binary_preds = cancer_mask(slideimg,he_mask)
...
#     print(cur_wd)
#     #Already prossessed files
#     prossessed_flist=sorted(glob.glob(cur_wd + '/Data/cancer_output/*png'))
#     prossessed_flist = [re.sub(cur_wd + '/Data/cancer_output/', "", x) for x in prossessed_flist]
